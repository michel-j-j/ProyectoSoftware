...
class DenunciaModel extends Model
{
    ...
    public function obtenerDenunciasActivas($idUsuario)
    {
        //recupera todas las denuncias activas
        $queryDocumentacion = $this->query("SELECT d.* FROM denuncia d 
        JOIN estado_denuncia ed ON ed.id = d.id_estado_denuncia 
        JOIN usuario u ON u.id = d.id_usuario
        WHERE u.id = $idUsuario AND (ed.descripcion = 'En curso' OR ed.descripcion = 'Iniciada');")->getResultArray();
        return $queryDocumentacion;
    }
}